...
def training_testing_accuracy():
    with open("../../results/prediction_accuracy.csv", "w") as f1:
        ...
        with open("../../results/techniques_accuracy_evaluation.csv", "w") as f2:
            f2.write("dataset,technique,real,pred,perc_outliers,std_outliers\n")
            ...
            for dataset in datasets:
                    for technique in techniques:
                        ...
                        knn = KNeighborsRegressor(n_neighbors=22, metric='manhattan')
                        ...
                        for i in range(0, len(y_test)):
                            f2.write(dataset + "_" + str(i) + "," + technique + "," + str(
                                y_test[i]) + "," + str(y_pred[i]) + "," + str(X_test_not_scaled.percentage_outliers[i]) + "," + str(X_test_not_scaled.std_outliers[i]) + "\n")
...
def evaluate_techniques():
    ...
    with open("../../results/techniques_accuracy_evaluation_total.csv", "w") as f:
        f.write("dataset,ranking_eval,ranking_real,ranking_pred,value_real,value_pred,perc_outliers,std_outliers\n")
        ...
        for dataset in datasets:
                ...
                f.write(dataset + "," + str(count / len(df_real)) + "," + str(
                    ...
                    np.array(df_real.real)).replace('\n', '') + "," + str(np.array(df_pred.pred)).replace('\n',
                                                                                                          '') + "," + str(rrr.perc_outliers[0]) + "," + str(rrr.std_outliers[0]) + "\n")
    print("Done!")
