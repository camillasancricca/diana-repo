...
selected_features = ['Strength','Intelligence','Weaknesses', name_class]
selected_features_only = ['Strength','Intelligence','Weaknesses']
...
def improve_accuracy(df, od_1, od_2, imp_1, imp_2, imp_3, imp_col_1, imp_col_2, imp_col_3, algorithm, name_class):
    df_clean = df[selected_features_only].copy()
...
def validate_sample(df, sample, algorithm, name_class):
    ...
    if sample.dimension_1 == 'completeness':
        ...
        df_clean, df_performance_1, df_performance_2 = improve_accuracy(df_clean, sample.od_1, sample.od_2, sample.imp_1, sample.imp_2, sample.imp_3, sample.imp_col_1, sample.imp_col_2, sample.imp_col_3, algorithm, name_class)
    else:
        df_clean, df_performance, df_performance_2 = improve_accuracy(df_clean, sample.od_1, sample.od_2, sample.imp_1, sample.imp_2, sample.imp_3, sample.imp_col_1, sample.imp_col_2, sample.imp_col_3, algorithm, name_class)
...
if __name__ == '__main__':
    ...
    sample_schedule = sample_schedule[76800:83200]
    ...
    sample_schedule.to_csv('schedule/compiled_sample_schedule_13.csv', index=False)
