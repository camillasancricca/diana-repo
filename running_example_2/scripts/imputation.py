...
from utils import encoding_categorical_variables, decode_categorical_variables
...
def impute(df, method, missing_column):
    ...
    if method == "no_impute":
        ...
        if len(imputated_df.columns) > 7:
            if missing_column == categorical_variables[0]:
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[1])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[2])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[3])

            elif missing_column == categorical_variables[1]:
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[0])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[2])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[3])

            elif missing_column == categorical_variables[2]:
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[0])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[1])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[3])

            elif missing_column == categorical_variables[3]:
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[0])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[1])
                imputated_df = decode_categorical_variables(imputated_df, categorical_variables[2])
